...
| Depends on | `api.anoda.com` (server-side only), TBD |
| Task alias | `portal-ui` (`task portal-ui:test`, `task portal-ui:lint`, `task portal-ui:dev`) |
| Host port | `${PORTAL_UI_PORT:-3000}` |

## Run locally

```bash
task portal-ui:install
task portal-ui:test
task portal-ui:lint
task portal-ui:dev
...
| `API_BASE_URL` | yes | — | Server-side base URL of `api.anoda.com` (never `NEXT_PUBLIC_`) |
...
Screen specs: `docs/design/screens/portal.ui.anoda.com/`. Shared components per `docs/design/README.md`.

## Runbook

- Health: `curl localhost:3000/healthz`
- Tailscale: `tailscale serve --bg --https=443 http://localhost:3000`
